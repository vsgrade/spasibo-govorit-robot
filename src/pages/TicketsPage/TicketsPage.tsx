...
import { getTickets, createTicket, Ticket as ApiTicket } from '../../services/ticketService';
...
export function TicketsPage() {
  ...
  const createTicketMutation = useMutation({
    ...
    onError: (err: Error) => {
      console.error("Ошибка создания тикета на фронтенде:", err);
  ...
  return (
    <div className="flex flex-col gap-6 p-4 md:p-6">
      ...
      {showCreateForm && (
        <Card>
          <CardHeader><CardTitle>Создать новый тикет</CardTitle></CardHeader>
          <CardContent>
            <form onSubmit={handleCreateTicketSubmit} className="space-y-4">
              <div>
                <Label htmlFor="title">Заголовок</Label>
                <Input id="title" value={newTicketTitle} onChange={(e) => setNewTicketTitle(e.target.value)} placeholder="Краткое описание проблемы" required />
              </div>
              <div>
                <Label htmlFor="description">Описание</Label>
                <Textarea id="description" value={newTicketDescription} onChange={(e) => setNewTicketDescription(e.target.value)} placeholder="Подробное описание проблемы" required />
              </div>
              <div>
                <Label htmlFor="clientName">Имя клиента</Label>
                <Input id="clientName" value={newTicketClientName} onChange={(e) => setNewTicketClientName(e.target.value)} placeholder="Иван Иванов" />
              </div>
              <div>
                <Label htmlFor="clientEmail">Email клиента</Label>
                <Input id="clientEmail" type="email" value={newTicketClientEmail} onChange={(e) => setNewTicketClientEmail(e.target.value)} placeholder="ivan@example.com" />
              </div>
      ...
      <Card>
        <CardHeader>
          ...
          <div className="flex justify-between items-center">
            ...
            <Button onClick={() => refetch()} disabled={isLoading} variant="outline">Обновить список</Button>
          </div>
        </CardHeader>
        <CardContent>
              ...
              {tickets.map((ticket) => (
                <Card key={ticket.id}>
                  <CardHeader>
                    ...
                    <CardDescription>ID: {ticket.id} | Приоритет: {ticket.priority}</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <p className="text-sm text-muted-foreground mb-2">Клиент: {ticket.client_name || 'N/A'} ({ticket.client_email || 'N/A'})</p>
