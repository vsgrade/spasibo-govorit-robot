...
/**
 * Интерфейс для карты разрешений пользователя
 */
interface PermissionMap {
  [UserRole.ADMIN]: string[];
  [UserRole.MANAGER]: string[];
  [UserRole.SUPPORT_AGENT]: string[];
  [UserRole.SALES_AGENT]: string[];
  [UserRole.READ_ONLY]: string[];
}

// Карта разрешений для различных ролей
const permissionMap: PermissionMap = {
  [UserRole.ADMIN]: ["*"], // Администратор имеет все права
  [UserRole.MANAGER]: [
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const hasPermission = (permission: string): boolean => {
    ...
    const userRole = UserRole.ADMIN; // Default to admin for now
