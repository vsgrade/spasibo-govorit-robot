  ...
  try {
    if (config.type === 'supabase') {
      if (!config.url || !config.apiKey) {
        return { success: false, message: 'URL и API ключ обязательны для Supabase' };
      }
      ...
      return { success: true, message: 'Успешное подключение к Supabase' };
    } else if (config.type === 'mysql') {
      // Для MySQL используем fetch API для тестирования
      const testData = {
        host: config.host,
        port: config.port,
      ...
      // Имитируем тест подключения (в реальном проекте здесь был бы API endpoint)
      if (!config.host || !config.database || !config.username) {
        return { success: false, message: 'Заполните все обязательные поля для MySQL' };
      }
      
      return { success: true, message: 'Настройки MySQL корректны (проверка через API недоступна в демо)' };
    }
    
    return { success: false, message: 'Неподдерживаемый тип базы данных' };
  } catch (error: any) {
